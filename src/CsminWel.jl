...
type MultivariateOptimizationResultsCs{T,N,M} <: OptimizationResults
      method::String
      initial_x::Array{T,N}
      minimizer::Array{T,N}
      minimum::Float64
...
function Optim.optimize(f::Function, initial_x::Array, method::Csminwel,
  ...
  csminwel(f, initial_x; H0 = H0, rng = rng, opts...)
end
...
function csminwel(fcn::Function,
    ...
    while !converged && iteration < iterations
        ...
        f1, x1, fc, retcode1 = csminit(fcn, x, f_x, gr, badg, H;
                                       verbose=verbose)
        f_calls += fc
...
=#
function csminwel(fcn::Function, x0::Vector; kwargs...)
    autodiff = false
    for fn in kwargs
        if fn[1] == :autodiff
            autodiff = fn[2]
